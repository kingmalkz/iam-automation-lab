...
**Updated:** Friday 18 September 2026 (rebuilt around your existing n8n stack from get.n8n.io; expected results for every step; Docker troubleshooting)
...
| **[n8n]** | n8n editor at `http://localhost:5678`, container `n8n-n8n-1` |
| **[Stack]** | The n8n Compose stack folder (`$STACK`), where `compose.yml` and `.env` live |
...
n8n was installed with the official script from `get.n8n.io`, so you already have a running Compose stack at `~/workspace/d_containers/d_n8n`. This roadmap refers to that folder as `$STACK` and uses it as-is instead of building a container from scratch.

| Service | Container | What it does |
|---|---|---|
| n8n | `n8n-n8n-1` | Editor, API, webhooks. The only service that should ever be reachable from outside |
| runners | `n8n-runners-1` | Runs Code node JavaScript in a separate process |
| sandbox-api | `n8n-sandbox-api-1` | Brokers sandboxed execution |
| sandbox-runner-1 | `n8n-sandbox-runner-1-1` | Docker-in-Docker sandbox, runs **privileged** |
| sandbox-certs | `n8n-sandbox-certs-1` | One-shot mTLS certificate job; `Exited (0)` is the correct state |
| searxng | `n8n-searxng-1` | Local metasearch engine that AI workflows can query |

Your workflows, credentials, and the SQLite database all live in one volume, `n8n_n8n-data`, mounted at `/home/node/.n8n`. Nothing else in the stack holds your work. Secrets (`N8N_ENCRYPTION_KEY`, `N8N_RUNNERS_AUTH_TOKEN`, `SEARXNG_SECRET`) live in `$STACK/.env`.

**`compose.yml` is managed by the installer and can be rewritten on upgrade.** Never edit it. All your changes go in `compose.override.yml`, which Compose merges automatically and the installer leaves alone. B1 creates that file and keeps it in your repo.

```bash
echo 'export STACK="$HOME/workspace/d_containers/d_n8n"' >> ~/.bashrc
source ~/.bashrc && ls "$STACK"
```

**Expected:** `compose.yml`, `.env`, and `searxng-settings.yml`.
...
├── infra/             # compose override and backup scripts (B1, E4)
...
| A3 | You are on n8n 2.39.7. In 2.x the Execute Command node is disabled by default, access to environment variables from nodes is blocked by default, and the old "Active" toggle may show as "Publish". This roadmap avoids Execute Command and turns on env access in E4. | Medium |
...
| A10 | You run Docker Engine (29.x) inside Ubuntu on WSL with systemd, not Docker Desktop. Troubleshooting is under B1, **If Docker fails**. | Low |
| A11 | n8n runs from the official `get.n8n.io` Compose stack in `$STACK`, with SQLite in the `n8n_n8n-data` volume until E4. Your own containers reach it over a separate Docker network called `automation`, which B1 connects. | **High.** Container names and paths throughout assume this. |
...
**Done when:** after `wsl --shutdown` and reopening Ubuntu, the stack comes back on its own, `curl` to the production webhook URL returns JSON, and the workflow JSON is on GitHub.

### Steps

1. **[WSL]** Confirm Docker works and find your stack.
   ```bash
   docker --version
   docker ps --format 'table {{.Names}}\t{{.Image}}\t{{.Status}}'
   ls "$STACK"
   ```
   *Why:* your n8n already runs, so B1 starts by confirming what you have instead of creating a second instance.

   **Expected:** Docker 29.x. A row for `n8n-n8n-1` running `n8nio/n8n:2.39.7` with STATUS `Up`, plus the runner, sandbox, and searxng containers. `n8n-sandbox-certs-1` showing `Exited (0)` is correct; it's a one-shot certificate job. If Docker commands fail, use **If Docker fails** at the end of B1. If no n8n container is running, start it with `cd "$STACK" && docker compose up -d`.

2. **[WSL]** Create the override file in your repo and link it into the stack.
   ```bash
   mkdir -p "$LAB/infra"
   cat > "$LAB/infra/compose.override.yml" << 'YML'
   # Local changes to the get-n8n.sh stack.
   # compose.yml is installer-managed; keep every edit here.
   services:
     n8n:
     ...
     searxng:
       restart: unless-stopped
   YML
   ln -sf "$LAB/infra/compose.override.yml" "$STACK/compose.override.yml"
   cd "$STACK" && docker compose config --quiet && echo CONFIG_OK
   docker compose up -d
   docker ps --format 'table {{.Names}}\t{{.Ports}}'
   ```
   *Why, line by line:* no service in the installer stack has a restart policy, so nothing comes back after a reboot and your scheduled workflows stop silently. The installer publishes 5678 on every interface; `127.0.0.1` limits it to this machine, which matters because n8n has no MFA in front of it. `!override` is needed because Compose merges list entries, which would otherwise keep both port bindings. Keeping the file in `$LAB/infra` and symlinking it means your change is versioned in Git and survives an installer upgrade.

   **Expected:** `CONFIG_OK`, then the n8n row shows `127.0.0.1:5678->5678/tcp` with no `0.0.0.0` entry. If `docker compose config` rejects `!override`, your Compose is older than that syntax; report it in the planning chat rather than editing `compose.yml`.

3. **[WSL]** Protect the secrets and take your first backup.
   ```bash
   cd "$STACK"
   chmod 600 .env
   grep -c N8N_ENCRYPTION_KEY .env
   docker run --rm -v n8n_n8n-data:/data -v "$PWD":/backup alpine \
     tar czf /backup/n8n-data-$(date +%F).tar.gz -C /data .
   ls -lh n8n-data-*.tar.gz
   ```
   Copy `N8N_ENCRYPTION_KEY` into your password manager now.
   *Why:* that key decrypts every stored credential. From B5 onward this volume holds an Entra client secret, so a backup and a safe copy of the key have to exist before then, not after.

   **Expected:** a `.tar.gz` of a few megabytes. If the `grep` prints `0`, n8n generated the key inside the volume at `/home/node/.n8n/config` instead; the backup covers it, and you can read it with `docker compose exec n8n cat /home/node/.n8n/config`.

4. **[WSL]** Join the shared network.
   ```bash
   docker network create automation 2>/dev/null
   docker network connect automation n8n-n8n-1
   docker inspect n8n-n8n-1 --format '{{range $k,$v := .NetworkSettings.Networks}}{{$k}} {{end}}'
   ```
   *Why:* the stack has its own Compose network. B6 runs your Python service as a separate container, and n8n reaches it by name only if both sit on `automation`. A container can be on several networks at once, so this doesn't disturb the stack.

   **Expected:** the format line prints both the Compose network (`n8n_default`) and `automation`. "already exists in network" is fine.

5. **[WSL]** Check the timezone setting.
   ```bash
   grep -E 'GENERIC_TIMEZONE|^TZ=' "$STACK/.env" || echo "not set"
   ```
   If nothing is set, add `GENERIC_TIMEZONE=Asia/Manila` and `TZ=Asia/Manila` to `$STACK/.env`, then `cd "$STACK" && docker compose up -d`.
   *Why:* schedule triggers fire in the instance timezone. Without this, B2's 18:00 digest arrives at the wrong hour.

   **Expected:** both variables present, and the Settings page in n8n shows the timezone as Asia/Manila.

6. **[Browser]** Open `http://localhost:5678`. Create the owner account if you haven't; otherwise sign in. Save the password in your password manager.

   **Expected:** the setup page on first use, then the Overview (workflows) page. If the page doesn't load, confirm `docker ps` shows `n8n-n8n-1` as `Up`, then try `http://127.0.0.1:5678`.
...
12. **[WSL]** Create the export script.
    ...
    ```
    *Why:* `REPO` is derived from the script's own location, so it runs from anywhere. The image filter finds `n8n-n8n-1` without hard-coding a name that Compose may change, and skips the runner and sandbox containers, which share similar names but can't export anything. Workflow JSON holds credential names and IDs, not secret values, so it's safe to commit.

    **Expected:** `Exported from n8n-n8n-1 to /home/mabiad/workspace/lab_ln/Experiments/iam-automation-lab/workflows`, and one `.json` file per workflow, each containing its `"name"`. "No running n8n container found" means the stack is down. `bad interpreter: ...^M` means Windows line endings; fix with `sed -i 's/\r$//' scripts/export-workflows.sh`.
...
14. **[PowerShell on Windows]** Run `wsl --shutdown`, reopen Ubuntu, make sure Docker is running, and repeat the production `curl`. This is the "done when" test.
    ...
    **Expected:** The same JSON as step 9, without starting anything by hand. This is what the restart policies in step 2 buy you. "Connection refused" means Docker or the stack didn't come back: check `systemctl status docker`, then `cd "$STACK" && docker compose up -d`.
...
6. **[WSL]** Build and run the container on the shared network, without publishing a port.
   ...
   docker exec n8n-n8n-1 wget -qO- http://auditor:5055/health
   ```
...
**Goal:** move your stack from SQLite to Postgres, add backups you have actually restored, add retries, and write a runbook. Every change goes through `compose.override.yml`, so an installer upgrade can't undo it.
**Done when:** the stack starts with one command, all earlier workflows run on Postgres with their credentials intact, a restore test succeeds, the backup job has run twice, and the runbook is enough for someone else to start the stack.

Use Claude Code to draft the files from this spec, then review them line by line.

### Steps

1. **[WSL]** Record what you're running before changing it.
   ```bash
   cd "$STACK"
   grep -E 'N8N_VERSION|N8N_SANDBOX_VERSION' .env
   docker compose ps
   ```
   *Why:* the installer already pins versions in `.env`. Write both values in `docs/runbook.md` so you can roll back to a known-good pair.

   **Expected:** two pinned versions (for example `N8N_VERSION=2.39.7`) and all services running, with the certificate job exited.

2. **[WSL]** Back up the volume and confirm your encryption key.
   ```bash
   cd "$STACK"
   docker run --rm -v n8n_n8n-data:/data -v "$PWD":/backup alpine \
     tar czf /backup/pre-postgres-$(date +%F).tar.gz -C /data .
   ```
   *Why:* this is your rollback for the whole migration. The key in `.env` must stay unchanged, because it's what makes your exported credentials importable into the new database.

   **Expected:** an archive of a few megabytes, and `N8N_ENCRYPTION_KEY` in your password manager from B1 step 3.

3. **[WSL]** Export workflows and credentials from the SQLite instance.
   ```bash
   cd "$STACK"
   docker compose exec n8n sh -c 'rm -rf /home/node/.n8n/exports && mkdir -p /home/node/.n8n/exports/workflows && \
     n8n export:workflow --all --pretty --separate --output=/home/node/.n8n/exports/workflows && \
     n8n export:credentials --all --output=/home/node/.n8n/exports/credentials.json'
   mkdir -p "$STACK/backups"
   docker compose cp n8n:/home/node/.n8n/exports/. "$STACK/backups/"
   ls -R "$STACK/backups"
   ```
   *Why:* exported credentials stay encrypted as long as you leave off `--decrypted`, and they import cleanly into the new database because the encryption key doesn't change. Never use `--decrypted`, and never commit `credentials.json`.

   **Expected:** `backups/workflows/` with one JSON per workflow, and `backups/credentials.json`. Opening the credentials file shows `"data"` as unreadable ciphertext. If it shows a readable client secret, you exported decrypted; delete the file and redo this step.

4. **[WSL]** Add Postgres and your settings to the override file.
   Append to `$LAB/infra/compose.override.yml` (the symlinked file from B1 step 2), keeping the services already there:
   ```yaml
     postgres:
       image: postgres:16
       restart: unless-stopped
       ...
       healthcheck:
         ...
         retries: 5
   ```
   and add to the `n8n` service block, alongside `restart` and `ports`:
   ```yaml
       depends_on:
         postgres:
   ...
   volumes:
     pg-data:
   ```
   *Why:* Compose merges mappings, so the existing `depends_on` on `sandbox-api` in `compose.yml` stays. The `./backups` mount is how the import in step 6 and the backup script in step 8 reach the container.

   **Expected:** `cd "$STACK" && docker compose config --quiet && echo CONFIG_OK` prints `CONFIG_OK`, and the resolved config shows both dependencies on the n8n service.

5. **[WSL]** Put the new settings in `$STACK/.env`. The n8n service reads that file, so everything goes in one place.
   ```
   POSTGRES_USER=n8n
   ...
   DB_POSTGRESDB_PASSWORD=<same value>
   EXECUTIONS_DATA_PRUNE=true
   EXECUTIONS_DATA_MAX_AGE=168
   N8N_BLOCK_ENV_ACCESS_IN_NODE=false
   LAB_TENANT_DOMAIN=YOURTENANT.onmicrosoft.com
   ```
   *Why each matters:* pruning deletes execution data older than 7 days, which limits how much personal data sits in the database. `N8N_BLOCK_ENV_ACCESS_IN_NODE=false` lets expressions read `LAB_TENANT_DOMAIN`, and it is only acceptable because no secret is stored in a variable that nodes can read. Never put a client secret in `.env` for that reason; secrets belong in n8n credentials.

   **Expected:** `.env` still has mode 600, and `git status` in `$LAB` never shows it (the file lives outside the repo).

6. **[WSL]** Start on Postgres and import.
   ```bash
   cd "$STACK" && docker compose up -d
   docker compose ps
   docker compose exec n8n n8n import:workflow --separate --input=/backups/workflows
   docker compose exec n8n n8n import:credentials --input=/backups/credentials.json
   ```
   Create the owner account again in the browser first if n8n asks for it; the new database starts empty.
   *Why:* import order doesn't matter, but both must happen before you test, because workflows reference credentials by ID.

   **Expected:** postgres shows `(healthy)`, n8n stays up, and both imports report the number of items. Every workflow appears in the editor with its credential attached and runs without you reselecting anything. If credentials fail to decrypt, the encryption key changed; restore the archive from step 2 and check `.env`.

7. **[WSL]** Delete the plaintext-adjacent export and recreate the API key.
   ```bash
   rm "$STACK/backups/credentials.json"
   ```
   In n8n, Settings, n8n API: create a new key (the old one lived in the SQLite database). Update `~/.config/lab/secrets.env`, then `claude mcp remove n8n-mcp` and add it again as in E3 step 3.

   **Expected:** `claude mcp list` shows n8n-mcp connected again, and Claude Code can list your workflows.

8. **[WSL]** Write `docs/credentials.md`: one row per credential with name, purpose, permissions, expiry date, and owner. No secret values.
   *Why:* this is the inventory a client or auditor asks for, and it tells you when the Entra client secrets from B5 and E1 expire.

   **Expected:** every credential in n8n appears in the table, including the two Entra apps and the n8n API key.

9. **[WSL]** Create `scripts/backup.sh`.
   ```bash
   #!/usr/bin/env bash
   # Nightly Postgres backup of the n8n stack, keeps the newest 14 files
   set -euo pipefail
   STACK="${STACK:-$HOME/workspace/d_containers/d_n8n}"
   cd "$STACK"
   set -a; source .env; set +a
   STAMP=$(date +%Y%m%d-%H%M)
   mkdir -p backups/db
   docker compose exec -T postgres pg_dump --no-owner -U "$POSTGRES_USER" "$POSTGRES_DB" | gzip > "backups/db/n8n-$STAMP.sql.gz"
   # Rotation: deletes everything older than the newest 14 backups
   ...
   ```
   *Why:* `--no-owner` keeps the dump restorable into a database that doesn't have your `n8n` role, which is what the restore test uses. 21:30 falls inside your study window, and cron in WSL only runs while WSL is open.

   **Expected:** a file like `n8n-20261120-2130.sql.gz` in `$STACK/backups/db/`, larger than 0 bytes. `crontab -l` shows the line, and the next day a second file exists with no errors in `backup.log`.

10. **[WSL]** Restore test. A backup you haven't restored is not a backup.
    ```bash
    docker run -d --name pg-restore-test -e POSTGRES_PASSWORD=test postgres:16
    sleep 10
    gunzip -c "$STACK"/backups/db/<latest>.sql.gz | docker exec -i pg-restore-test psql -U postgres
    docker exec pg-restore-test psql -U postgres -c "select count(*) from workflow_entity;"
    docker rm -f pg-restore-test
    ```
    *Why:* restoring into a throwaway container proves the dump is complete without touching your live database.

    **Expected:** many `CREATE` and `ALTER` lines, then a count matching the number of workflows in your editor. Record the date and count in `docs/runbook.md`.

11. **[n8n]** On every Graph HTTP Request node: Settings, **Retry On Fail**, 3 tries, 5000 ms wait. Replace the hard-coded domain in E1 with `{{ $env.LAB_TENANT_DOMAIN }}`. Confirm the error workflow from E2 is set on every active workflow.
    *Why:* Graph returns occasional throttling (429) and transient server errors. Retries handle those without waking you.

    **Expected:** each Graph node shows Retry On Fail turned on, and an E1 test run builds the UPN from the environment variable.

12. **[Claude Code]** Draft `docs/runbook.md`, then correct it yourself. It must cover: start and stop (`cd "$STACK" && docker compose up -d` / `down`); that `compose.yml` is installer-managed and every change belongs in `$LAB/infra/compose.override.yml`, symlinked in; upgrading (back up, run the installer's upgrade, then `docker compose up -d`, and check the override still applies); restoring from a dump; rotating an Entra client secret; rotating the n8n API key; and where the encryption key is kept.
...
13. **[WSL]** Run every earlier workflow once, then export, commit, push.

    **Expected:** every workflow has a successful run on the new database, and `git status` is clean apart from expected changes.
...
5. **[WSL]** Create database `posture` in the stack's Postgres (`cd "$STACK" && docker compose exec -it postgres psql -U n8n -d n8n`) with tables `tenants` (`tenant_id`, `name`, `report_email`, `active`) and `snapshots` (`tenant_id`, `taken_at`, `metrics jsonb`). Use a least-privilege user, as in X1.
